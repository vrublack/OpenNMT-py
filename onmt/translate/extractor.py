...
def build_extractor(opt, logger=None, out_file=None):
    assert len(opt.models) == 1
...
class Extractor(object):
    ...
    def extract(
        ...
        for batch in data_iter:
            encoder_states = self.extract_batch(
                batch, data.src_vocabs, attn_debug
            )

            encoder_states = self.unsort(encoder_states, batch.indices)

            for sample_i in range(encoder_states.shape[0]):
                formatted = self.hidden2str(encoder_states[sample_i])
                self.out_file.write(formatted + '\n')

    def hidden2str(self, hidden):
        return ' '.join(['{:.5f}'.format(f) for f in hidden.tolist()])

    def unsort(self, encoder_states, indices):
            # Sorting
            inds, perm = torch.sort(indices)
            return encoder_states.index_select(0, perm)
    ...
    def extract_batch(self, batch, src_vocabs, attn_debug):
    ...
    def _extract_batch(
        ...
        src, enc_states, memory_bank, src_lengths = self._run_encoder(batch)
        # have to write sentence, enc_state[0]
        self.model.decoder.init_state(src, memory_bank, enc_states)

        # return last layer
        return enc_states[0][-1, :, :]
